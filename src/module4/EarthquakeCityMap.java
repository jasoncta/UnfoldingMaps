...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() {
		...
		int totalO = quakeMarkers.size();

		// TODO: Implement this method
		for (Marker i : countryMarkers) {
			int counter = 0;
			
			String country = i.getStringProperty("name");

			for (Marker j : quakeMarkers) {
				EarthquakeMarker m = (EarthquakeMarker) j;
				if (m.isOnLand) {
					if (country.equals(m.getStringProperty("country"))) {
			...
			if (counter > 0) {
				...
				totalO = totalO - counter;

			}

		}

		System.out.println("Ocean: " + totalO);
	}
